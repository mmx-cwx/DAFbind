...
import torch
from model import m_model_esm_650m_64 as m_model
...
def collate_fn(data):
    ...
    hhm_fea = np.load('../../prot_t5/DNA573/'+name+'.npy')
    hhm_fea = torch.tensor(hhm_fea).to('cuda')

    return input, labels ,len(labels) , tokens,hhm_fea
...
def test(args,test_file):
    ...
    for fold in range(0,5):
        ...
        with (torch.no_grad()):
            ...
            for j, (input, labels, length, seq, hhm_fea) in loop:
                out,l = model(input, seq=seq, prot_feature=hhm_fea)
                out = out.squeeze(0)
                # 归一化
...
if __name__ =='__main__':
    ...
    parser.add_argument('--save_model',type=str,default='../attention_layer/model_181_129/')
    parser.add_argument('--train_data',type=str,default='./dataset/DNA_Train_573.csv')
    ...
    #test(args, "dataset/tsne_exp.csv")
